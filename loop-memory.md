...
### R6 (2026-07-15) — 微信公众号图片版 2026 真题抓取

- **来源**：Sogou 微信搜索 `weixin.sogou.com/weixin?type=2&query=...`
- **技术要点**：
  - `/link?url=...` 是一次性签名 URL，必须搜索后**立即**跟随（同一 cookie 会话）
  - 页面用 JS `url += 'https://mp.weixin...'` 拼接真实 URL，需正则提取拼接
  - mmbiz.qpic.cn 图片有防盗链，需带 `Referer: https://mp.weixin.qq.com/` 头
- **抓取脚本**：`/tmp/wx_batch.py`（按学科分批调用，规避 2 min bash 超时）
- **搜索关键词**：`2026北京中考{学科}真题`、`2026北京中考{学科}试卷答案` 等
- **过滤规则**：必须含 `2026`+`北京`+`中考`，剔除广东/广州/区名/一模/期末/暑假/模拟/听说考等
- **去重策略**：跨学科命中的通用文（如"全科目答案速览"）统一移到 `overview/`
- **精减策略**：无试题图（>50KB）的营销/评论文一律删除；数学冗余保留题量最丰富的 4 篇
- **最终产物**：`papers/downloaded/2026/{学科}/*.html + *_imgs/*.png/jpg`
  - math 4 篇（每篇 12-17 张试题扫描图，含官方标答）
  - overview 6 篇（含全科目答案速览）
  - physics 2 篇（深度解析+试题图）
  - chemistry/chinese/english/history/politics 各 1-2 篇（内容较少）
  - biology / geography：0 篇（自媒体几乎不发这两科真题）
- **重要发现**：**bjeea.cn 官方 2026 真题至今未公布**（历史规律：2024 年真题在考后 3 个月才发布，2025 年真题目前仍未发布）
- **`zizzs.com` / `gaokzx.com`（2025 主源）现均 404**
- **总数变化**：PDF 52 → 52，HTML 16 → 35（+19 2026），图片 0 → 203 张
- **前端更新**：`build_downloads.py` 增加 `_imgs` 目录过滤 + 图片包附加信息展示
