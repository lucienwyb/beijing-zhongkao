...
// Total days derived from plan data — changing curriculum.js plan length
// automatically updates all bounds below. No magic 21 to hunt down.
const TOTAL_DAYS=mathPlans.length;
state.completed=Array.isArray(state.completed)?state.completed.filter(n=>Number.isInteger(n)&&n>=0&&n<TOTAL_DAYS):[];
state.mistakes=Array.isArray(state.mistakes)?state.mistakes:[];
state.selectedDay=Number.isInteger(state.selectedDay)&&state.selectedDay>=0&&state.selectedDay<TOTAL_DAYS?state.selectedDay:0;
...
function renderProgress(){const count=state.completed.length,pct=Math.round(count/TOTAL_DAYS*100);$('completedDays').textContent=count;$('progressPercent').textContent=`${pct}%`;$('progressMessage').textContent=count===TOTAL_DAYS?'三周训练完成，保持整卷节奏。':count>=14?'进入整卷阶段，重点控制非知识性失分。':count>=7?'基础框架已建立，开始突破中档模型。':'先完成诊断，找到最值得补的分数。'}
...
$('prevDay').onclick=()=>{state.selectedDay=(state.selectedDay+TOTAL_DAYS-1)%TOTAL_DAYS;state.selectedWeek=Math.floor(state.selectedDay/7);save();renderToday()};
$('nextDay').onclick=()=>{state.selectedDay=(state.selectedDay+1)%TOTAL_DAYS;state.selectedWeek=Math.floor(state.selectedDay/7);save();renderToday()};
